#!/usr/bin/env bash
# Self-test for validate-versions.py: exercises pass + both fail paths against
# synthetic git fixtures. Run: bash .github/actions/version-validator/test-validate-versions.sh
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
VALIDATOR="${SCRIPT_DIR}/validate-versions.py"
PASS=0
FAIL=0
...
python3 "$VALIDATOR" --repo-root "$R1" >/dev/null 2>&1
...
python3 "$VALIDATOR" --repo-root "$R2" >/dev/null 2>&1
...
python3 "$VALIDATOR" --repo-root "$R3" --base-ref "$BASE3" >/dev/null 2>&1
...
python3 "$VALIDATOR" --repo-root "$R4" --base-ref "$BASE4" >/dev/null 2>&1
...
python3 "$VALIDATOR" --repo-root "$R5" --base-ref "$BASE5" >/dev/null 2>&1
